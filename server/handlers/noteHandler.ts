...
const create = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!title || !content) {
      return next(new AppError('Title and content are required', 400, 'Bad Request'))
    }
...
const getMyAll = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    res.status(200).json({ status: 'Success', results: docs.length, notes: docs })
  } catch (err) {
    console.log(err)
    next(err)
  }
...
const deleteMy = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const doc = await Note.findOneAndDelete({ _id: req.params.id, authorId: authorId })
    if (!doc) {
      return next(new AppError('Note not found', 404, 'Not Found'))
    }
